...
class API {
  ...
  /**
   * Updates the expiration date of the data with the given IDs by the
   * specified number of days.
   *
   * To decrease the lifespan of the data, provide a negative number. Note that
   * if the expiration date of a data after modification is in the past, the
   * data will be deleted.
   *
   * @async
   ...
   * @param {number} days - the number of days by which to extend the lifespan
   *   of the data
   * @return {Object} an obect mapping data IDs to true/false values indicating
   *   whether the data was successfully processed (true)
   * @throws {APIError} if the request was unsuccessful
   */
  async batchUpdateDataTTL(dataIds, days) {
    return await this.batchRequest_('data', 'ttl', dataIds, {days});
  }
   ...
   * @return {Object} an obect mapping job IDs to true/false values indicating
  ...
  /**
   * Updates the expiration date of the jobs with the given IDs by the
   * specified number of days.
   *
   * To decrease the lifespan of the jobs, provide a negative number. Note that
   * if the expiration date of a job after modification is in the past, the job
   * will be deleted.
   *
   * @async
   * @param {Array} jobIds - the job IDs
  ...
  async batchUpdateJobsTTL(jobIds, days) {
    return await this.batchRequest_('jobs', 'ttl', jobIds, {days});
  }
