...
class API {
   ...
   * @param {boolean} [isImageToVideo=false] - whether the analytic that you
   *  are uploading is a image-based model for use with the Image-To-Video tool
   * @returns {object} an object containing metadata about the posted analytic
   * @throws {APIError} if the request was unsuccessful
   */
  async uploadAnalytic(docJSONPath, isImageToVideo = false) {
    let uri = this.baseURL + '/analytics';
    let formData = {
      file: fs.createReadStream(docJSONPath),
    };
    if (isImageToVideo) {
      formData['is_image_to_video'] = isImageToVideo.toString();
    }
   ...
   * @async
   * @param {string} jobId - the job ID
   * @param {string} outputPath - the path to write the logfile
   * @throws {APIError} if the request was unsuccessful
   */
   ...
   * @async
   * @param {string} jobId - the job ID
   * @returns {string} a signed URL with read access to download the job
   *   logfile
   * @throws {APIError} if the request was unsuccessful
   */
   ...
   *
   * @returns {Object} an object describing the status of the platform
   */
