...
class API {
  ...
  /**
   * Updates the expiration date of all specified data by the specified number
   * of days. See updateDataTTL for details.
   *
   * @async
   ...
   * @throws {APIError} if the request was unsuccessful
   */
  async batchUpdateDataTTL(dataIds, days) {
    return await this._batchRequest('data', 'ttl', dataIds, {days});
  }
  ...
  async batchDeleteData(dataIds) {
    return await this._batchRequest('data', 'delete', dataIds);
  }
  ...
  async waitUntilJobCompletes(jobId, sleepTime=5, maxWaitTime=600) {
    ...
    return await utils.waitForCondition(
  ...
  async batchStartJobs(jobIds) {
    return await this._batchRequest('jobs', 'start', jobIds);
  }
  ...
  async batchArchiveJobs(jobIds) {
    return await this._batchRequest('jobs', 'archive', jobIds);
  }
  ...
  async batchUnarchiveJobs(jobIds) {
    return await this._batchRequest('jobs', 'unarchive', jobIds);
  }

  /**
   * Updates the expiration date of all specified jobs by the specified number
   * of days. See updateJobTTL for details.
   *
   * @async
   * @param {Array} jobIds - the job IDs
   * @param {number} days - the number of days by which to extend the lifespan
   ...
   * @throws {APIError} if the request was unsuccessful
   */
  async batchUpdateJobsTTL(jobIds, days) {
    return await this._batchRequest('jobs', 'ttl', jobIds, {days});
  }

  /**
   * Deletes all jobs with the given IDs. See deleteJob for details.
   *
   * @async
   * @param {Array} jobIds - the job IDs
   * @throws {APIError} if the request was unsuccessful
  ...
  async batchDeleteJobs(jobIds) {
    return await this._batchRequest('jobs', 'delete', jobIds);
  }

  /**
   * Force kills all jobs with the given IDs. See killJob for details.
   *
   * @async
   * @param {Array} jobIds - the job IDs
   * @throws {APIError} if the request was unsuccessful
  ...
  async batchKillJobs(jobIds) {
    return await this._batchRequest('jobs', 'kill', jobIds);
  }
  ...
  async _batchRequest(type, action, ids, params={}) {
    let uri = urljoin(this.baseURL, type, 'batch');
    let bodyData = Object.assign({}, params, {action, ids});
    return await requests.post(uri, this.header_, {
      json: true,
      body: bodyData,
    });
  }
