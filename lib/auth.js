...
function activateToken(path) {
  ...
  fs.copyFileSync(path, TOKEN_PATH);
}
...
function loadToken(tokenPath = null) {
  ...
  return Token.fromDisk(path);
}

/**
 * Class encapsulating an API authentication token.
 *
 * @class
 */
class Token {
   ...
   * @retun {Token} an instance of Token
   */
   ...
   * @return {Token} an instance of Token()
   */
  static fromDisk(path) {
    return Token(utils.readJSON(path));
  }
}
