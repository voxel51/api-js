 ...
 */
function waitForCondition(condition, sleepTime, maxWaitTime) {
  ...
  let checkCondition = function(resolve, reject) {
    condition.then(function(result) {
      if (result) {
        ...
        reject(new APITimeoutError('Maximum wait time exceeded'));
      }

      // Wait for awhile before checking condition again
      setTimeout(checkCondition, sleepTime, resolve, reject);
...
function constructQueryUri({fields, sort, searches, limit} = {}) {
  return new Promise(function(resolve, reject) {
    let fieldString = addFields();
    let sortString = addSort();
    let searchString = addSearches();
    let limitString = addLimit();
    resolve('?'.concat(
      fieldString,
      sortString,
      searchString,
  ...
  function addFields() {
    return fields.reduce(function addField(string, field, index) {
      string += `${field}`;
  ...
  function addSearches() {
    return searches.reduce(function addSearch(string, search, index) {
      string += `${search.field}:${search.substring}`;
