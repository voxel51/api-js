...
class API {
  ...
  /**
   * Uploads a private analytic JSON file which describes the
   * analytic interface and function.
   *
   * @async
   * @param {String} path - the local disk path to file
   * @throws {APIError} if the request was unsuccessful
   */
  async uploadAnalytic(path) {
    let formData = {
  ...
  /**
   * Uploads the image(s) for the private analytic. The type refers
   * to either 'cpu' or 'gpu'.
   *
   * @async
   * @param {String} analyticId - the analytic ID
   * @param {String} path - the local disk path to image file
   * @param {String} imageType - image computation type (cpu | gpu)
   * @throws {APIError} if the request was unsuccessful
   */
  async uploadAnalyticImage(analyticId, path, imageType) {
    if (typeof imageType !== 'string' ||
      imageType.toLowerCase() !== 'cpu' ||
      imageType.toLowerCase() !== 'gpu') {
      throw new utils.InvalidInputError(`Invalid image type specified: ${imageType}.`);
    }
    let formData = {
      file: fs.createReadStream(path),
    };
    let uri = this.baseURL + '/analytics/' + analyticId +
      '/images?type=' + imageType.toLowerCase();
    await requests.post(uri, this.header_, {formData; formData});
  }
