...
class API {
   ...
   * @param {Boolean} supportsCPU - whether the analytic supports CPU execution
   * @param {Boolean} supportsGPU - whether the analytic supports GPU execution
  ...
  async uploadAnalytic(docJSONPath, supportsCPU, supportsGPU) {
    ...
    let body = await requests.post(
      uri, this.header_, {formData: formData, qs: params});
  ...
  /**
   * Downloads the logfile of the job with the given ID. This
   * function is only available for private analytics.
   *
   * @async
   * @param {string} jobId - the job ID
   * @param {string} outputPath - the output to write to
   * @returns {void} when the download completes
   * @throws {APIError} if the request was unsuccessful
   */
  async downloadJobLogfile(jobId, outputPath) {
    let stream = fs.createWriteStream(outputPath);
    stream.on('error', function(error) { throw error; });
    stream.on('end', function() { return; });
   ...
   * This function is only available for private analytics.
   *
   * @async
   * @param {string} jobId - the job ID
   * @returns {string} a signed URL with read access to download job logfile
   * @throws {APIError} if the request was unsuccessful
   */
