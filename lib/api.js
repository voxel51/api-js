...
class API {
   ...
   *
   * @async
   * @param {object} analyticsQuery An AnalyticsQuery object defining the
   *   customzied analytics query to perform
   * @returns {Array} an array of objects describing the query results
   * @throws {APIError} if the request was unsuccessful
   */
  async queryAnalytics(analyticsQuery) {
    let uri = this.baseURL + '/analytics';
    let body = await requests.get(
      uri, this.header_, {qs: analyticsQuery.toString()});
    return JSON.parse(body).analytics;
  }
  ...
  /**
   * Generic query route for data that provides filtering returned fields,
   * sorting ascending/descending by field, substring searching on field(s),
   * and limiting the permitted quantity of returned entries.
   *
   ...
   *  expires
   *
   * A default set of parameters is provided that gets all available fields,
   * sorted by descdending chronological order, and limited to the first 100
   * results.
   *
   * @async
   * @param {object} qsObj Querystring object with fields (required: Array),
   * sort (optional: String), search (optional: Array), and limit
   * (optional: Number) properties.
   * @returns {object} Object with an array of the returned query entries.
   * @throws {APIError} if the request was unsuccessful
   */
  async queryData(qsObj = {
    fields: ['id', 'name', 'encoding', 'type', 'size', 'date', 'expires'],
    sort: 'date:desc',
    search: [],
    limit: 100,
  }) {
    let uri = this.baseURL + '/data';
    let body = await requests.get(uri, this.header_, {qs: qsObj});
    return JSON.parse(body).data;
  }
  ...
  async waitUntilJobCompletes(jobId, sleepTime = 5, maxWaitTime = 600) {
    return utils.waitForCondition(
      this.isJobComplete(jobId), 1000 * sleepTime, 1000 * maxWaitTime
    );
  }
  ...
  /**
   * Generic query route for jobs that provides filtering returned fields,
   * sorting ascending/descending by field, substring searching on field(s),
   * and limiting the permitted quantity of returned entries.
   *
   * Permitted fields are:
   ...
   * @throws {APIError} if the request was unsuccessful
   */
  async queryJobs(qsObj = {
    fields: ['id', 'name', 'state', 'archived', 'date'],
    sort: 'date:desc',
    search: [],
    limit: 100,
  }) {
